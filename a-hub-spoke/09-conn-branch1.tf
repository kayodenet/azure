...
locals {
  branch1_nva_init = templatefile("../scripts/nva-branch.sh", {
    ...
    TUNNELS = [
      {
        ike = {
          ...
          dest    = azurerm_public_ip.hub1_vpngw_pip0.ip_address
        },
        ipsec = {
          peer_ip = azurerm_public_ip.hub1_vpngw_pip0.ip_address
          psk     = local.psk
        }
      },
      {
        ike = {
          ...
          source  = local.branch1_nva_ext_addr
          dest    = azurerm_public_ip.hub1_vpngw_pip1.ip_address
        },
        ipsec = {
          peer_ip = azurerm_public_ip.hub1_vpngw_pip1.ip_address
          psk     = local.psk
        }
      },
    ]
    ...
    BGP_SESSIONS = [
      { peer_asn = local.hub1_vpngw_asn, peer_ip = local.hub1_vpngw_bgp0, source_loopback = true, ebgp_multihop = true },
      { peer_asn = local.hub1_vpngw_asn, peer_ip = local.hub1_vpngw_bgp1, source_loopback = true, ebgp_multihop = true },
    ]
